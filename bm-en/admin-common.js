...
(function () {
  ...
  /* ─── Notification data ──────────────────────────────── */
  const notifications = [
    { type: 'warn', icon: '⚠️', msg: 'BuildUp Gangnam Dispenser #3 low stock (12% remaining)', time: '5 min ago', unread: true },
    { type: 'info', icon: '📦', msg: 'BuildUp Seongsu restock complete (Protein Powder and 3 others)', time: '23 min ago', unread: true },
    { type: 'err',  icon: '🔴', msg: 'BuildUp Seodaemun Kiosk K-02 disconnected', time: '1 hour ago', unread: false },
    { type: 'info', icon: '🛒', msg: '7 new Store orders received', time: '2 hours ago', unread: false },
    { type: 'warn', icon: '🔔', msg: 'Pre-order open D-1 reminder', time: 'Yesterday', unread: false },
  ];

  const branches = [
    'BuildUp Seongsu', 'BuildUp Gangnam', 'BuildUp Seodaemun', 'BuildUp Jamsil', 'BuildUp Bundang', 'BuildUp Incheon',
  ];
  ...
  function buildNotifDrop() {
    ...
    return `<div class="tb-dropdown notif-drop" id="notifDrop">
      ...
      <div class="notif-drop-footer"><a href="admin-notification.html">View all notifications →</a></div>
    </div>`;
  }

  function buildBranchDrop() {
    const current = document.querySelector('.topbar-branch')?.textContent?.replace('▾','').trim() || 'BuildUp Seongsu';
    ...
    return `<div class="tb-dropdown branch-drop" id="branchDrop">
      <div class="branch-drop-label">Select branch</div>
      ${items}
    </div>`;
  }
    ...
    return `<div class="tb-dropdown user-drop" id="userDrop">
      <div class="user-drop-header">
        ...
        <div>
          ...
          <div class="user-drop-role">Branch Admin</div>
        </div>
      </div>
      ...
      <div class="user-drop-divider"></div>
      <div class="user-drop-item danger" onclick="if(confirm('Log out?')) location.href='admin-index.html'">
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    <div class="cm-modal-overlay" id="cmProfileModal">
      <div class="cm-modal">
        <div class="cm-modal-header">
          <span class="cm-modal-title">Profile settings</span>
        ...
        <div class="cm-avatar-row">
          ...
          <div>
            <button class="cm-btn cm-btn-outline" style="font-size:12px;padding:7px 14px;" onclick="document.getElementById('cmAvatarInput').click()">Change photo</button>
            <input id="cmAvatarInput" type="file" accept="image/*" style="display:none" onchange="previewCmAvatar(this)">
            <div style="font-size:11px;color:#A0A8B3;margin-top:5px;">JPG, PNG · Max 2MB</div>
          </div>
        </div>
        <div class="cm-form-group">
          <label class="cm-form-label">Name</label>
        ...
        <div class="cm-form-group">
          <label class="cm-form-label">Role</label>
          <input class="cm-form-input" value="Branch Admin" readonly style="background:#F8F8FA;color:#767676;" />
        </div>
        <div class="cm-modal-footer">
          <button class="cm-btn cm-btn-outline" onclick="document.getElementById('cmProfileModal').classList.remove('open')">Cancel</button>
          <button class="cm-btn cm-btn-primary" onclick="saveCmProfile()">Save</button>
        </div>
      </div>
    </div>`;
    ...
    <div class="cm-modal-overlay" id="cmPasswordModal">
      <div class="cm-modal">
        <div class="cm-modal-header">
          <span class="cm-modal-title">Change password</span>
          <button class="cm-modal-close" onclick="document.getElementById('cmPasswordModal').classList.remove('open')">✕</button>
        </div>
        <div class="cm-form-group">
          <label class="cm-form-label">Current password</label>
          <input class="cm-form-input" id="cmPwCurrent" type="password" placeholder="Enter current password" />
        </div>
        <div class="cm-form-group">
          <label class="cm-form-label">New password</label>
          <input class="cm-form-input" id="cmPwNew" type="password" placeholder="8+ characters, letters + numbers" />
        </div>
        <div class="cm-form-group">
          <label class="cm-form-label">Confirm new password</label>
          <input class="cm-form-input" id="cmPwConfirm" type="password" placeholder="Re-enter new password" />
          <span class="cm-form-hint" id="cmPwHint"></span>
        </div>
        <div class="cm-modal-footer">
          <button class="cm-btn cm-btn-outline" onclick="document.getElementById('cmPasswordModal').classList.remove('open')">Cancel</button>
          <button class="cm-btn cm-btn-primary" onclick="saveCmPassword()">Change</button>
        </div>
      </div>
    </div>`;
  ...
  window.saveCmPassword = function () {
    ...
    if (!current) { hint.textContent = 'Please enter your current password.'; hint.style.color = '#EC3D3D'; return; }
    if (newPw.length < 8) { hint.textContent = 'Password must be at least 8 characters.'; hint.style.color = '#EC3D3D'; return; }
    if (newPw !== confirm) { hint.textContent = 'New passwords do not match.'; hint.style.color = '#EC3D3D'; return; }
    hint.textContent = 'Password changed.'; hint.style.color = '#00B377';
  ...
  // filter-select-btn arrow auto-handling
  document.addEventListener('DOMContentLoaded', function() {
  ...
  // Global function that changes only the filter-select-btn text while keeping the SVG
